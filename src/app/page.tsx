...
export default function Home() {
  ...
  return (
    <div className="main">
      ...
      {/* <section 
        id="hero-section" 
        className="flex flex-col md:flex-row items-center justify-center xl:gap-12 py-12 md:py-44 
          bg-radial from-violet-400 from-30% to-zinc-200 dark:bg-none dark:bg-transparent"
        >  */}  
        <section id="hero-section" 
        className="flex flex-col md:flex-row items-center justify-center xl:gap-12 py-12 md:py-44 
          bg-cover bg-fixed bg-header">     
        <div className="text-center flex flex-col items-center md:items-start pb-6 md:pb-0 m-5">
          <h1 className="2xl:mx-42 text-4xl md:text-5xl lg:text-6xl font-bold text-center pb-6 leading-12 md:leading-20 text-white">
            Hi <span className="wave-hover">👋</span> My name is Thainá
            and I'm a <span className="rounded-2xl p-1 bg-gradient-to-r from-purple-500 to-blue-500 whitespace-nowrap text-white">Full Stack Developer</span>
          </h1>
          <span className="w-full text-sm md:text-lg text-center max-w-8/10 md:max-w-full text-white">who also loves <span className="text-violet-400 font-bold">art,</span> <span className="text-sky-500 font-bold">travelling,</span> <span className="text-emerald-400 font-bold">the nature</span> and <span className="text-yellow-500 font-bold">biking!</span></span>
          <div className="flex flex-row gap-2 pt-8 w-full items-center justify-center">
            <Link href="/contact" className="bg-primary py-2 px-8 md:px-12 border border-primary rounded-md font-medium cursor-pointer text-sm md:text-base text-white hover:bg-primary-dark hover:border-primary-dark">Contact</Link>
            <Link href="/projects" className="py-2 px-3 md:px-8 border border-zinc-400 rounded-md font-medium cursor-pointer text-sm md:text-base hover:bg-background hover:dark:bg-card text-white hover:text-foreground">View Projects</Link>
          </div>
        </div>
      </section>
